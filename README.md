...
Turn on **Captions Overlay** in the menu to see the latest lines over any app while listening, even a call in full screen. Drag it anywhere and resize it to show more or less; your typing stays in the call. Its translation button shows the original, the translation, or both, apart from the window, and ✕ hides it until you next start listening. With it on, starting from the menu bar opens no window, and starting from the window closes it so the call comes forward. **Settings > General** sets its text size. People see the overlay when you share your whole screen; share a single window to keep it to yourself.
