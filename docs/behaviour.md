...
## AppKit: floating panels

- **Over another app's full-screen Space, a regular app's panel needs `.nonactivatingPanel`.** On macOS 26, a `.floating` `NSPanel` with `[.canJoinAllSpaces, .fullScreenAuxiliary]` is not shown there while its app's activation policy is `.regular`; with `.nonactivatingPanel` in its style mask, it is. Reported, with tests over full-screen TextEdit and Chrome, in [classroom-widgets #162](https://github.com/tinkertanker/classroom-widgets/pull/162); the same fix is in [SwitchHosts #1051](https://github.com/oldj/SwitchHosts/pull/1051) and [speaktype #142](https://github.com/karansinghgit/speaktype/pull/142).
- **`sharingType = .none` does not keep a window out of capture.** Apple's documentation calls it "a legacy constant that macOS no longer uses"; on macOS 15.4 and later, ScreenCaptureKit captures such a window ([Apple Developer Forums 792152](https://developer.apple.com/forums/thread/792152)).
